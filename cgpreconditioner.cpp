...
static vector<pair<int, float> >conda;
...
//ofstream myfile1 ("ap.txt");
	...
	for (map<int,matrix_x*>::iterator it = matrix_mapx.begin(); it!= matrix_mapx.end(); it++)
	{
		for (list<pair<int, float> > :: iterator it1 = it->second->movelist.begin(); it1!= it->second->movelist.end(); ++it1)
		{
			...
			{
//				cout << "map data is " << it1->second << endl;
				a = float (1/it1->second);
				conda.push_back(pair<int, float> (it->first, a));
			}
		}
	}
...
do
{
	...
	{
		numerator = numerator + ((it->second->r)*(it->second->z));
	...
	for (map<int,param*> ::iterator it=par_map.begin(); it!= par_map.end(); ++it)
        {
                znew = 0;
                znew = float ((it->second->r)*(conda[it->first].second));
                par_map[it->first]->z = znew;
	}

	numerator1 = 0;
	for (map<int,param*> ::iterator it = par_map.begin(); it!= par_map.end(); it++)
	{
		numerator1 = numerator1 + ((it->second->r)*(it->second->z));
	}
	...
	for (map<int,param*> ::iterator it=par_map.begin(); it!= par_map.end(); ++it)
        {
		pnew = 0;
                pnew = float ((it->second->z) + (beta*(it->second->p)));
