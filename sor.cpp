...
static map <int, float> solold_map;
static map <int, float> diff_map;
...
float n1, n2;
float omega = 0.9;
ofstream myfile ("initsolution_x.txt");
...
do
{
	...
	for (map<int,matrix_x*>::iterator it = matrix_mapx.begin(); it!= matrix_mapx.end(); it++)
	{
		...
		n2 = 0;
		check = check + 1;
		solold_map[it->first] = sol_map[it->first];
		for (list<pair<int, float> > :: iterator it1 = it->second->movelist.begin(); it1!= it->second->movelist.end(); ++it1)
		{
			if ((it1->first) != (it->second->row))
			{
				n1 = float (n1 + ((it1->second)*(sol_map.find(it1->first)->second)));		
			}
			else
			{
				n2 = float (it1->second);
		...
		xnew = float ((1-omega)*(sol_map.find(it->first)->second)) + ((omega/n2)*((it->second->data)-n1));
		sol_map[it->first] = xnew;
	}
	
	for (int i = 0; i < countmove ; i++)
	{
		xdiff1 = float ( (sol_map[i] - solold_map[i]));
		diff_map[i] = xdiff1/solold_map[i];
	}
	...
	cout << "iteration = " << iteration << endl;
      ...
      for (map<int,float>::iterator it=sol_map.begin(); it!= sol_map.end(); ++it)
        {
		myfile << "map id is: " << it->first << "\t";
		myfile << "soln is: " << it->second << endl;
	}
...
//		cout << "degree = " << degree << endl;
		...
		std::sort(nodexpos.begin(), nodexpos.end(), sort_nodes);
		present = 0;
...
do
{
	...
	for (map<int,matrix_y*>::iterator it = matrix_mapy.begin(); it!= matrix_mapy.end(); it++)
	{
		...
		ynew = float ((1-omega)*(sol_map.find(it->first)->second)) + ((omega/n2)*((it->second->data)-n1));
		sol_map[it->first] = ynew;
	}
	
	for (int i = 0; i < countmove ; i++)
	{
		ydiff1 = float ( (sol_map[i] - solold_map[i]));
		diff_map[i] = ydiff1/solold_map[i];
	}
